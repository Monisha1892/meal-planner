...
const IngredientDetailsScreen = ({ route, navigation }) => {
  const { ingredientId } = route.params; // Get ingredient ID from navigation parameters
  ...
  useEffect(() => {
    ...
    const fetchIngredientDetails = async () => {
      try {
        const response = await fetch(`http://192.168.56.1:3011/ingredients/${ingredientId}`);
        if (!response.ok) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.nutritionText}>Nutritional Information: {ingredient.nutritional_information}</Text>
